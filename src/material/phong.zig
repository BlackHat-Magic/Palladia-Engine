...
pub const phong_material_instanced_vert_spv align(4) = @embedFile("../shaders/spirv/phong_material_instanced.vert.spv");
...
pub fn createPhongMaterialInstanced(
    ...
    const vertex_shader = try loadShaderFromBytes(
        device,
        phong_material_instanced_vert_spv,
        sdl.SDL_GPU_SHADERSTAGE_VERTEX,
        .{ .sampler_count = 0, .uniform_buffer_count = 1, .storage_buffer_count = 1 },
    );
    errdefer sdl.SDL_ReleaseGPUShader(device, vertex_shader);
